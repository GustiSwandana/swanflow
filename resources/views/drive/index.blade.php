    ...
    <div class="relative bg-gradient-to-br from-slate-900 via-teal-950 to-slate-900 px-5 pt-8 pb-10 text-white overflow-hidden shadow-lg border-b border-teal-900/40">
        ...
        <div class="relative z-10 flex items-center justify-between mb-4">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-teal-200/80 font-medium">Penyimpanan & Transfer Berkas Mandiri</p>
                </div>
            </div>
        ...
        <div class="relative z-10 bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/15 shadow-inner">
            ...
            <div class="flex items-center justify-between text-[10px] text-teal-200/70 mt-1.5 font-medium">
                ...
                <span>Transfer Berkas Cepat</span>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<div class="bg-slate-50 dark:bg-slate-900 rounded-t-[32px] pt-4 px-4 pb-24 shadow-2xl -mt-4 relative z-10 border-t border-slate-200 dark:border-slate-800 min-h-screen space-y-5 text-slate-800 dark:text-white transition-colors">
    <!-- Drag Handle -->
    ...
    <div class="bg-white dark:bg-slate-950 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
        <form id="upload-form" action="{{ route('drive.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
            ...
            <div id="dropzone" onclick="document.getElementById('upload-input').click()"
                ...
                <div class="w-11 h-11 rounded-full bg-teal-100 dark:bg-teal-900/50 text-teal-600 dark:text-teal-400 flex items-center justify-center mb-2 shadow-xs">
                ...
                <span id="dropzone-text" class="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Sentuh untuk pilih file atau seret file ke sini
                </span>
                <span class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                    Mendukung PDF, Word, Excel, Gambar, ZIP, CSV, dll. (Maks 50 MB)
                </span>
            </div>
    ...
    <div class="space-y-2.5">
        ...
        <form method="GET" action="{{ route('drive.index') }}" class="relative">
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama file, ekstensi, atau catatan..."
            ...
            @if(!empty($search))
                <a href="{{ route('drive.index', ['category' => $activeCategory]) }}" class="absolute right-2.5 top-2.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-xs font-bold">
                    ✕
                </a>
            @endif
        </form>
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5 -mx-1 px-1">
            <a href="{{ route('drive.index', ['category' => 'all', 'search' => $search]) }}"
            ...
            <a href="{{ route('drive.index', ['category' => 'document', 'search' => $search]) }}"
            ...
            <a href="{{ route('drive.index', ['category' => 'image', 'search' => $search]) }}"
            ...
            <a href="{{ route('drive.index', ['category' => 'archive', 'search' => $search]) }}"
            ...
            <a href="{{ route('drive.index', ['category' => 'other', 'search' => $search]) }}"
                ...
                <div class="flex items-start justify-between gap-3">
                    ...
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-1 text-[10px] text-slate-400 dark:text-slate-500 font-medium">
            ...
            <div class="text-center py-12 px-4 bg-white dark:bg-slate-950 rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 space-y-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-xs mx-auto">
                        Unggah file dokumen, foto kuitansi, sertifikat, atau arsip Anda untuk disimpan privat atau ditransfer kapan saja.
                    </p>
                </div>
...
<!-- 4. SHARE & TRANSFER BOTTOM SHEET MODAL -->
            ...
            <div id="active-share-section" class="space-y-3">
                <div>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <button type="button" onclick="copyShareLink()" class="px-3 py-2 rounded-xl bg-teal-500 hover:bg-teal-600 text-white text-xs font-bold active:scale-95 transition-all shrink-0 flex items-center gap-1 shadow-sm">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                ...
                <!-- Action Buttons: WhatsApp & Open in New Tab -->
...
<script>
    ...
    function copyShareLink() {
        ...
        navigator.clipboard.writeText(input.value).then(() => {
            ...
            document.getElementById('copy-text').innerText = 'Tersalin!';
            setTimeout(() => {
                document.getElementById('copy-text').innerText = 'Salin';
            }, 2000);
        });
    }
</script>
